...
export default function ProcedureMode({ disease }: { disease: DiseaseData }) {
  ...
  useEffect(() => {
    ...
    async function loadProcedure() {
      // 切换疾病/手术时先清掉上一页数据，避免短暂显示上一个疾病的手术内容。
      setProcedureData(null);
      ...
      if (error) {
        ...
        const nextProcedure = (data?.data || null) as ProcedureData | null;
        const relatedDiseaseIds = Array.isArray(nextProcedure?.relatedDiseaseIds) ? nextProcedure?.relatedDiseaseIds || [] : [];

        // 数据库也做一道归属校验：Procedure 若明确声明 relatedDiseaseIds，
        // 必须包含当前疾病，避免错误引用把 A 病的手术显示到 B 病。
        if (nextProcedure && relatedDiseaseIds.length > 0 && !relatedDiseaseIds.includes(disease.id)) {
          setProcedureData(null);
          setLoadError("该手术数据与当前疾病不匹配，已阻止显示。请执行 Procedure 关联修复 SQL。");
        } else {
          setProcedureData(nextProcedure);
        }
      }
      setLoading(false);
    }
  ...
  return (
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 space-y-5">
        ...
        {!loading && !loadError && !procedureData && (
          <Card title="该疾病的手术内容待补充" tone="warning">
            <p className="text-sm leading-7 text-[var(--of-warning-text)]">当前不会借用其他疾病的 Procedure。可以先保留该疾病自己的手术策略概览，随后再逐项补充真实的入路、解剖危险区、手术步骤、器械和影像复查要点。</p>
          </Card>
        )}
